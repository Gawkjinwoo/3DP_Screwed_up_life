...
#define PAUSE_DATA_SIGNAL_PIN      12
...
float P_GAIN_PITCH = 0.8, I_GAIN_PITCH = 0.05, D_GAIN_PITCH = 55;
float P_GAIN_ROLL = 0.8, I_GAIN_ROLL = 0.05, D_GAIN_ROLL = 55;
float P_GAIN_YAW = 0.1, I_GAIN_YAW = 0.1, D_GAIN_YAW = 0.1;
...
void setup(){
  ...
  pinMode(PAUSE_DATA_SIGNAL_PIN, INPUT);
...
void loop(){
  ...
  if(millis() >= 20000 && millis() <= 22000){
    //Serial.print("MOTOR!!!!!!!!!!!!!");
    MOTOR_1.writeMicroseconds(1450);
    MOTOR_2.writeMicroseconds(1450);
    MOTOR_3.writeMicroseconds(1450);
    MOTOR_4.writeMicroseconds(1450);
  }
  else{
  ...
  Serial.print("Controlled value roll: "), Serial.print(CONTROLLED_VALUE_ROLL); 
  Serial.println();
  PAUSE_DATA();
}
//--------------------------------------------------//
...
//--------------------------------------------------//
void GET_ANGLE(){
  double DT = (TIME_NOW_ANGLE - TIME_LAST_ANGLE)/1000.0;
...
void UPDATE_MOTOR(){
  MOTOR_OUTPUT_1 = constrain(1450 + CONTROLLED_VALUE_PITCH - CONTROLLED_VALUE_ROLL, MIN_SIGNAL, MAX_SIGNAL);
  MOTOR_OUTPUT_2 = constrain(1450 - CONTROLLED_VALUE_PITCH + CONTROLLED_VALUE_ROLL, MIN_SIGNAL, MAX_SIGNAL);
  MOTOR_OUTPUT_3 = constrain(1450 - CONTROLLED_VALUE_PITCH - CONTROLLED_VALUE_ROLL, MIN_SIGNAL, MAX_SIGNAL);
  MOTOR_OUTPUT_4 = constrain(1450 + CONTROLLED_VALUE_PITCH + CONTROLLED_VALUE_ROLL, MIN_SIGNAL, MAX_SIGNAL);
